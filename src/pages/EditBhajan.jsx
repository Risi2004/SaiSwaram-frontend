...
import { getBhajanById, saveBhajan } from '../data/offlineRepository';
...
function EditBhajan() {
  ...
  useEffect(() => {
    const fetchBhajan = async () => {
      try {
        ...
        const existingLines = data.lyrics.split('\n');

        setFormData({
          title: data.title,
          deity: data.deity,
          pitch: data.pitch || '',
          lineCount: existingLines.length.toString()
        });
  ...
  const handleChange = (e) => {
    ...
    // Dynamically adjust lines if lineCount is directly manually edited
    if (name === 'lineCount') {
      const count = parseInt(value, 10);
      const newLines = isNaN(count) || count < 0 ? [] : new Array(count).fill('');
      
      // Preserve existing text if shrinking/growing
      for(let i=0; i<Math.min(newLines.length, lines.length); i++){
        newLines[i] = lines[i];
      }
      setLines(newLines);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await saveBhajan({ id, data: bhajanData });
      navigate('/dashboard');
    } catch (err) {
      setError(err.message);
    } finally {
  ...
  if (isLoading) {
      return <div className="min-h-screen flex items-center justify-center font-sans"><p className="text-xl text-gray-500">Loading editor...</p></div>;
  }
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col font-sans selection:bg-primary selection:text-white">
      ...
      <main className="flex-1 py-12 px-6">
        <div className="max-w-2xl mx-auto bg-white rounded-3xl p-8 md:p-10 shadow-lg border border-gray-100">
          <h1 className="text-3xl font-extrabold text-gray-900 mb-2">Edit Bhajan</h1>
          <p className="text-gray-500 mb-8">Update your bhajan details below.</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="pt-6">
              ...
              >
                {isSubmitting ? 'Updating...' : 'Update Bhajan'}
              </button>
            </div>
